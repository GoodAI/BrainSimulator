...
extern "C"  
{
						...
						)
	{
		...
		{				
			...
			{
				float inputValue = s_input.Ptr[x];
				float weightValue = s_weight.Ptr[neuronId + x * s_output.Count];
								...
								)
	{
		...
		if(inputId < s_previousLayerDelta.Count)
		{
			...
			for (int i = 0; i < s_delta.Count; i++)
			{
				...
				float weightValue = s_weight.Ptr[i + inputId * s_delta.Count];
						...
						)
	{
		...
		if(weightId < s_weightChange.Count)
		{
			...
			float deltaValue = s_delta.Ptr[weightId % s_delta.Count];

			int inputNeuronId = weightId / s_delta.Count;

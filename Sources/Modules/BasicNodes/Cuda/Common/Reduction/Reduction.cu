...
__forceinline__ __device__ void DReduction(void* rawOut, volatile const void* rawIn, unsigned int size, unsigned int outOff, unsigned int inOff, unsigned int stride)
{
    ...
    R sum;
    for (unsigned int i = stride * (blockIdx.x * tCnt + tid); i < size; i += stride * tCnt * gridDim.x)
    {
        sum.op(in[i], i + inOff);
    }
    sPartials[tid] = sum;
    ...
    LogStepShared<R,tCnt,false>(&out[blockIdx.x], sPartials);
    ...
    {
        R sum;
        for (unsigned int i = tid; i < gridDim.x; i += tCnt)
        {
            sum.op(out[i]);
        }
...
template<typename R, typename T, unsigned int tCnt>
__global__ void Reduction(void* rawOut, volatile const void* rawIn, unsigned int size, unsigned int outOff, unsigned int inOff, unsigned int stride)
{
    DReduction<R,T,tCnt>(rawOut, rawIn, size, outOff, inOff, stride);
}

template<typename R, typename T, unsigned int tCnt>
__forceinline__ __device__ void DDotProduct(void* rawOut, unsigned int outOff, volatile const void* rawIn1, volatile const void* rawIn2, unsigned int size)
{
    R* out = reinterpret_cast<R*>(buffer);
    ...
    R sum;
    for (unsigned int i = blockIdx.x * tCnt + tid; i < size; i += tCnt * gridDim.x)
    {
...
template<typename R, typename T, unsigned int tCnt>
__global__ void DotProduct(void* rawOut, unsigned int outOff, volatile const void* rawIn1, volatile const void* rawIn2, unsigned int size)
{
    DDotProduct<R,T,tCnt>(rawOut, outOff, rawIn1, rawIn2, size);
}

// I am currently using only 1024-thread version
template<typename R, typename T>
void ReductionTemplate()
{
    Reduction<R,T,1024><<<0,0>>>(0,0,0,0,0,0);
}

template<typename R, typename T>
void DotProductTemplate()
{
    DotProduct<R,T,1024><<<0,0>>>(0,0,0,0,0);
}
...
typedef void(*reduction)(void*, volatile const void*, unsigned int, unsigned int, unsigned int, unsigned int);

template<typename R, typename T>
void TestReduction(reduction kernel, const char* name, int repetitions, int sizeMax, int min, int max, float div)
{
    const int w = 20;
    for (int r = 0; r < repetitions; ++r)
    {
        ...
        kernel<<<10,1024,sizeof(R)*1024>>>(d_out, d_in, size, outOff, inOff, stride);
        ...
        bool passed = R::check(h_out, c_out, outOff, h_in);
        printf("------------\n");
...
typedef void(*dotproduct)(void*, unsigned int, volatile const void*, volatile const void*, unsigned int);

template<typename R, typename T>
void TestDotProduct(dotproduct kernel, const char* name, int repetitions, int sizeMax, int min, int max, float div)
{
    const int w = 20;
    for (int r = 0; r < repetitions; ++r)
    {
        ...
        kernel<<<10,1024,sizeof(R)*1024>>>(d_out, outOff, d_in1, d_in2, size);
        ...
        printf("Check (GPU == CPU): \n");
        bool passed = R::check(h_out, c_out, outOff, h_in1, h_in2);
        printf("------------\n");
        if (passed) printf("|  PASSED  |\n");
        else printf("|  FAILED  |\n");
        printf("------------\n\n");
...
int main(int argc, char* argv[])
{
    ...
    // INTEGER BASED
    TestReduction<i_Sum_i, int>(Reduction<i_Sum_i, int, 1024>, "Reduction i_Sum_i", repetitions, sizeMax, -10, 10, 1);
    TestReduction<i_MinIdx_2i, int>(Reduction<i_MinIdx_2i, int, 1024>, "Reduction i_MinIdx_2i", repetitions, sizeMax, -10000, 10000, 1);
    TestReduction<i_MaxIdx_2i, int>(Reduction<i_MaxIdx_2i, int, 1024>, "Reduction i_MaxIdx_2i", repetitions, sizeMax, -10000, 10000, 1);
    TestReduction<i_MinIdxMaxIdx_4i, int>(Reduction<i_MinIdxMaxIdx_4i, int, 1024>, "Reduction i_MinIdxMaxIdx_4i", repetitions, sizeMax, 0, 10000, 1);

    // // SINGLE BASED
    TestReduction<f_Sum_f, float>(Reduction<f_Sum_f, float, 1024>, "Reduction f_Sum_f", repetitions, sizeMax, -100, 100, 100);
    TestReduction<f_MinMax_2f, float>(Reduction<f_MinMax_2f, float, 1024>, "Reduction f_MinMax_2f", repetitions, sizeMax, -100000, 100000, 1000);
    TestReduction<f_MinIdx_fi, float>(Reduction<f_MinIdx_fi, float, 1024>, "Reduction f_MinIdx_fi", repetitions, sizeMax, -100000, 100000, 1000);
    TestReduction<f_MaxIdx_fi, float>(Reduction<f_MaxIdx_fi, float, 1024>, "Reduction f_MaxIdx_fi", repetitions, sizeMax, -100000, 100000, 1000);
    TestReduction<f_MinIdxMaxIdx_fifi, float>(Reduction<f_MinIdxMaxIdx_fifi, float, 1024>, "Reduction f_MinIdxMaxIdx_fifi", repetitions, sizeMax, 0, 100000, 1000);

    // // DOT PRODUCT
    TestDotProduct<i_Dot_i, int>(DotProduct<i_Dot_i, int, 1024>, "DotProduct i_Dot_i", repetitions, sizeMax, -10, 10, 1);
    TestDotProduct<f_Dot_f, float>(DotProduct<f_Dot_f, float, 1024>, "DotProduct f_Dot_f", repetitions, sizeMax, -100, 100, 100);
    TestDotProduct<f_Cosine_f, float>(DotProduct<f_Cosine_f, float, 1024>, "DotProduct f_Cosine_f", repetitions, sizeMax, -100, 100, 100);
